...
(Appuyer sur le bouton "Show" pour voir le code)
```{r}
...
Voici un tableau qui définit les différents MOS (Mode d'Occupation des Sols).
...
On modifie notre jeu de données en ajoutant les distances des stations par rapport au centre ville de Bordeaux. 
(Appuyer sur le bouton "Show" pour voir le code)
...
Nous retrouvons donc la même observation qu'avec la variable d'artificialisation des sols (MOS11), car l'artificialisation des sols et la distance avec le centre ville sont positivement corrélées (peut-etre essayer de mesurer la correlation entre ces deux variables sur nos données)
...
Voici, ci-dessous le Sun Burst Plot intéractif.
Vous pouvez cliquer sur les différentes années puis sur les différents MOS pour zoomer et obtenir plus d'informations sur la présence des espèces d'oiseaux dans un MOS d'une année spécifique. 
La présence d'une espèce d'oiseau dans un MOS est calculée en fonction du nombre d'oiseaux observés dans une parcelle multiplié par le pourcentage du MOS en question dans la parcelle. Les parcelles ont été filtré en ne gardant que celles avec un buffer size de 500.
...
- Nous voulons savoir quels espèces d'oiseaux étaient présentes dans le MOS 11 en 2023. 
Pour cela, il suffit de cliquer au niveau de "2023" et de cliquer ensuite sur "MOS11" pour obtenir les espèces d'oiseaux présentent.
Ceci est très utile lorsqu'on veut étudier la répartition des espèces d'oiseau ou regarder la présence d'une espèce dans un MOS et une année que l'on veut étudier. On peut aussi
Pour finir l'exemple, on remarque que l'espèce Passer Domesticus est la plus présente dans le MOS 11 en 2023, avec 13% des observations. On retrouve ensuite les espèces Sturnus vulgaris et Columba livia avec respectivement 11% et 8% des observations.
Presque 1/4 des oiseaux observés dans le MOS 11 en 2023 ont une présence inférieure à 2%. Ce sont donc des espèces moins communes dans ce MOS.

<center>
  ![Zoom sur le Sunburst pour l'année 2023 et le MOS 11](./images/sunburst2023MOS11.png)
</center>
