...
Dans le cadre de notre projet de statistiques, nous avons décidé de travailler sur un jeu de données environnementales.
...
Dans cette partie, nous allons explorer la mesure de la diversité à travers différentes entropies.
Nous nous intéresserons uniquement au MOS11, c'est à dire les surfaces artificialisées.

Dans un premier temps, nous allons utiliser le nombre d’espèces différentes observées dans une station pour mesurer la diversité.
Dans un deuxième temps, nous allons utiliser l’entropie de Shannon et enfin l'indice de Simpson.
...
Voici le graphique qui montre la relation entre la diversité et MOS11 pour chaque année expliqué par le nombre d'espèces.
```{r}
ggplot(index, aes(x = MOS11, y = D1, color = as.factor(annee))) +
  geom_point(size = 2) +
  geom_smooth(method = "auto", se = TRUE, color = "black", alpha = 0.2) +
  labs(title = " 'Nombre d\'espèces' en fonction de MOS11",
       x = "MOS11",
       y = "Nombre d\'espèces") +
  theme_minimal() +
...
La tendance globale est l'augmentation de la diversité au début de la courbe. C'est à dire que, moins les sols sont 
artificialisés, plus la diversité est grande. 
En effet, puisqu'on peut remarquer que plus l'artificialisation est forte, moins la diversité est grande.
